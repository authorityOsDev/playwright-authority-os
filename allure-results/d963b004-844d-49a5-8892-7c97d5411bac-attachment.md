...
```
Error: ENOTDIR: not a directory, open '.auth/user.json'
```
...
```ts
  ...
  6  |  * This special test logs in once and saves the browser state 
  7  |  * to a file for reuse in all other tests.
  8  |  */
  9  | setup('authenticate', async ({ page }) => {
  10 |   const loginPage = new LoginPage(page);
  11 |   const storagePath = process.env.STORAGE_STATE || '.auth/user.json';
  12 | 
  13 |   // Navigate and login
  14 |   await loginPage.goto();
  15 |   await loginPage.login(
  16 |     process.env.STANDARD_USER!, 
  17 |     process.env.PASSWORD!
  18 |   );
  19 | 
  20 |   // Verify successful login by checking the URL
  21 |   await expect(page).toHaveURL(/inventory.html/);
  22 | 
  23 |   // Save the authenticated state (cookies, localStorage)
> 24 |   await page.context().storageState({ path: storagePath });
     |   ^ Error: ENOTDIR: not a directory, open '.auth/user.json'
  25 |   
  26 |   console.log(`✅ Auth state saved to: ${storagePath}`);
  27 | });
```